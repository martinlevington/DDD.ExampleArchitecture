...
using DDD.Wpf.Common.Application;
using DDD.Wpf.Common.Data;
using DDD.Wpf.Common.Windows.Events;
using DDD.Wpf.Common.Windows.Models;
using DDD.Wpf.Common.Windows.ViewModels;
using DDD.Wpf.Common.Windows.Views;
using DryIoc;
using Prism.Events;
using Prism.Regions;
    ...
    public class WindowService : IWindowService
    {
        ...
        private Window CreateWindow(Type type, WindowKey windowKey, Action<IResultDataCollection> onWindowClosedAction, WindowKey parentWindowKey, IRegionManager regionManager)
        {
            ...
            if (regionManager != null)
            {
                ...
                using (var scope = _container.OpenScope())
                {
                    ...
                    window =  scope.Resolve(type)  as Window;;
                }
        ...
        private void AskToSaveChanges(WindowInformation windowInformation)
        {
            var navigationParameters = new NavigationParameters
            {
                ...
                {nameof(ModalButton), new ModalButton[] { ModalButton.Yes, ModalButton.No }},
                {nameof(ModalIcon), ModalIcon.Warning}
            };

            OpenDialog(windowInformation.Window, nameof(ModalView), navigationParameters, args =>
            {
                var result = (ModalButton)args.GetResults().First().Results[0];
